    ...
    <div className="flex flex-col flex-1 w-full max-w-container-max-width mx-auto px-margin-mobile md:px-margin-desktop py-8 gap-6">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-title-lg font-bold text-on-surface">Gestion des professeurs</h1>
        </div>
        <button 
          onClick={() => handleOpenModal()} 
          className="bg-germany-red hover:bg-red-700 text-white px-4 py-2.5 rounded-xl font-bold text-sm transition-colors flex items-center justify-center gap-2 shadow-sm"
        >
                ...
                <tr key={u._id} className="border-b border-surface-variant last:border-0 hover:bg-surface-container/50 transition-colors">
                  <td className="p-4 font-bold text-on-surface cursor-pointer group" onClick={() => setViewingUser(u)}>
                    <div className="flex items-center gap-3">
                      {u.photo ? (
                        <img src={u.photo} alt={u.name} className="w-10 h-10 rounded-full object-cover border border-surface-variant shrink-0 group-hover:ring-2 ring-germany-red transition-all" />
                      ) : (
                        <div className="w-10 h-10 rounded-full bg-surface-variant flex items-center justify-center text-secondary border border-surface-variant shrink-0 group-hover:ring-2 ring-germany-red transition-all">
        ...
        {totalPages > 1 && (
          <div className="sticky bottom-0 z-20 p-4 border-t border-surface-variant flex flex-col md:flex-row items-center justify-between gap-4 bg-surface-container-lowest/95 backdrop-blur-md rounded-b-xl shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
            <span className="text-sm text-secondary font-medium">
               Affichage {((currentPage - 1) * itemsPerPage) + 1} à {Math.min(currentPage * itemsPerPage, filteredUsers.length)} sur {filteredUsers.length} professeurs
            </span>
            <div className="flex gap-1 overflow-x-auto max-w-full pb-1 md:pb-0">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                <button
                  ...
                  className={`w-8 h-8 shrink-0 flex items-center justify-center rounded-lg text-sm font-medium transition-colors ${currentPage === page ? 'bg-germany-red text-white border-germany-red' : 'border border-surface-variant text-secondary hover:bg-surface-variant hover:text-on-surface'}`}
                >
                  {page}
                </button>
              ))}
          ...
          <div className="bg-surface w-full max-w-2xl rounded-3xl p-6 md:p-8 max-h-[90vh] overflow-y-auto relative">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <input 
                    ...
                    className="flex-1 px-4 py-2 rounded-xl bg-surface-container border border-surface-variant file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-germany-red file:text-white hover:file:bg-red-700 transition-all" 
                  />
              ...
              <div className="md:col-span-2 mt-4">
                <button type="submit" disabled={uploadingImage} className={`w-full bg-germany-red text-white py-3 rounded-xl font-bold transition-colors ${uploadingImage ? 'opacity-50 cursor-not-allowed' : 'hover:bg-red-700'}`}>
